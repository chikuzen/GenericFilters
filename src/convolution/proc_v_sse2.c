                 ...
                 int height, int stride, uint8_t *dstp, const uint8_t *srcp)
{
    ...
    line_copy8(p0, srcp, width, 0);
    line_copy8(p1, srcp, width, 0);
    srcp += stride;
    ...
    for (int y = 0; y < height; y++) {
        line_copy8(p2, srcp, width, 0);

        for (int x = 0; x < width; x += 16) {
            __m128i sum[4] = { zero, zero, zero, zero };
            uint8_t *array[] = { p0 + x, p1 + x, p2 + x };

            for (int i = 0; i < 3; i++) {
                __m128i xmm0, xmm1, xmm2;

                xmm0 = _mm_load_si128((__m128i *)array[i]);
...
static void GF_FUNC_ALIGN VS_CC
proc_3_9_10_sse2(convolution_t *ch, uint8_t *buff, int bstride, int width,
                 int height, int stride, uint8_t *d, const uint8_t *s)
{
    ...
    line_copy16(p0, srcp, width, 0);
    line_copy16(p1, srcp, width, 0);
    srcp += stride;
    ...
    for (int y = 0; y < height; y++) {
        line_copy16(p2, srcp, width, 0);

        for (int x = 0; x < width; x += 8) {
            __m128i sum[2] = { zero, zero };
            uint16_t *array[] = { p0 + x, p1 + x, p2 + x };
...
static void GF_FUNC_ALIGN VS_CC
proc_5_9_10_sse2(convolution_t *ch, uint8_t *buff, int bstride, int width,
                 int height, int stride, uint8_t *d, const uint8_t *s)
{
    const uint16_t *srcp = (uint16_t *)s;
    uint16_t *dstp = (uint16_t *)d;
