            ...
            const uint8_t *r, uint16_t max)
{
    ...
    for (int y = 0; y < h; y++) {
        dstp[0] = clamp_f((r[0] * m0 + r[0] * m1 + r[1] * m2) * rdiv + bias, max);
        for (int x = 0; x < w; x++) {
            dstp[x] = clamp_f((r[x - 1] * m0 + r[x] * m1 + r[x + 1] * m2) * rdiv + bias, max);
        }
        dstp[w] = clamp_f((r[w_] * m0 + r[w] * m1 + r[w] * m2) * rdiv + bias, max);
        r += stride;
        dstp += stride;
    }
}
             ...
             const uint8_t *srcp, uint16_t max)
{
    ...
    for (int y = 0; y < h; y++) {
        dstp[0] = clamp_d((r[0] * m0 + r[0] * m1 + r[1] * m2) * rdiv + bias, max);
        for (int x = 0; x < w; x++) {
            dstp[x] = clamp_d((r[x - 1] * m0 + r[x] * m1 + r[x + 1] * m2) * rdiv + bias, max);
        }
        dstp[w] = clamp_d((r[w_] * m0 + r[w] * m1 + r[w] * m2) * rdiv + bias, max);
        r += stride;
        dstp += stride;
    }
}
            ...
            const uint8_t *r, uint16_t max)
{
    ...
    for (int y = 0; y < h; y++) {
        ...
        dstp[1] = clamp_f(value * rdiv + bias, max);
        ...
        value = r[w_ - 2] * m0 + r[w_ - 1] * m1 + r[w_] * m2 + r[w] * m3 +
                r[w] * m4;
        dstp[w_] = clamp_f(value * rdiv + bias, max);
             ...
             const uint8_t *srcp, uint16_t max)
{
    ...
    for (int y = 0; y < h; y++) {
        ...
        value = r[0] * m0 + r[0] * m1 + r[1] * m2 + r[2] * m3 + r[3] * m4;
        dstp[1] = clamp_d(value * rdiv + bias, max);
        ...
        value = r[w_ - 2] * m0 + r[w_ - 1] * m1 + r[w_] * m2 + r[w] * m3 +
                r[w] * m4;
        dstp[w_] = clamp_d(value * rdiv + bias, max);
